...
class R1Model(tf.keras.Model):
    def __init__(self):
        ...
        self.r1 = [
            # Block 1
            Conv2D(16, 9, 1, padding="same", name="block1_conv1", input_shape = (100, 100, 1)),
...
def main():
    ...
    image_set =  np.zeros((len(images), 100, 100))
    for i, image in enumerate(images):
        im = resize(image, (100, 100))
        image_set[i] = im

    image_set = image_set.reshape((len(image_set), 100, 100, 1))

    train_data = image_set[:300, :, :, :] #temporary fix
    test_data = image_set[300:, :, :, :]


    density_train= image_set[:300, :, :, :]
    density_test = image_set[300:, :, :, :]

    dens_train = np.zeros((300, 25, 25, 1))
    dens_test = np.zeros((len(images)-300, 25, 25, 1))

    for i in range(len(density_train)):
        dens_train[i]= resize(density_train[i], (25, 25, 1))
    
    for i in range(len(density_test)):
        dens_test[i]= resize(density_test[i], (25, 25, 1))

    density_train = dens_train
    density_test = dens_test
    ...
    for model in networks:
        ...
        model.fit(
            ...
            y = density_train,
            validation_data=(test_data, density_test),
            epochs=hp.num_epochs,
            callbacks=callback_list,
            batch_size = None
        )
